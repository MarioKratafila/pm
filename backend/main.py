...
def init_db() -> None:
    with contextlib.closing(get_db_connection()) as connection:
        ...
        if "password_hash" not in {row[1] for row in cursor.fetchall()}:
        ...
        if not boards_exists:
            ...
            if "name" not in {row[1] for row in cursor.fetchall()}:
                cursor.execute(
...
def insert_default_board(connection: sqlite3.Connection, user_id: int, name: str = "My Board") -> int:
    cursor = connection.cursor()
    cursor.execute(
...
def get_or_create_default_board(
    ...
    return insert_default_board(connection, user_id), DEFAULT_BOARD
...
async def register(request: RegisterRequest) -> dict[str, str]:
    ...
    with contextlib.closing(get_db_connection()) as connection:
        if get_user_by_username(connection, request.username):
        ...
        connection.commit()
        insert_default_board(connection, user_id)

    return {"token": issue_session_token(request.username), "username": request.username}


@app.post("/api/login")
async def login(request: LoginRequest) -> dict[str, str]:
    is_default_login = request.username == "user" and request.password == "password"

    with contextlib.closing(get_db_connection()) as connection:
        user = get_user_by_username(connection, request.username)

        if user is None:
            if not is_default_login:
...
async def create_board(
    ...
    with contextlib.closing(get_db_connection()) as connection:
        user_id = get_or_create_user_id(connection, username)
        board_id = insert_default_board(connection, user_id, name)
...
async def rename_board(
    ...
    with contextlib.closing(get_db_connection()) as connection:
        user_id = get_or_create_user_id(connection, username)
        if not rename_board_by_id(connection, board_id, user_id, name):
            raise HTTPException(status_code=404, detail="Board not found")
        return {"status": "ok"}
...
async def proxy_ai(
    ...
    output["updatedBoard"] = board_dict
    with contextlib.closing(get_db_connection()) as connection:
        user_id = get_or_create_user_id(connection, username)
        if request.board_id is not None:
            save_board_by_id(connection, request.board_id, user_id, board_dict)
        else:
            board_id, _ = get_or_create_default_board(connection, user_id)
            save_board_by_id(connection, board_id, user_id, board_dict)
